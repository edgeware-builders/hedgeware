...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		load_spec(id, self.run.parachain_id.unwrap_or(2036).into())
	}
	...
	match &cli.subcommand {
		...
		Some(Subcommand::ExportGenesisState(params)) => {
			...
			let block: crate::service::Block = generate_genesis_block(&load_spec(
				...
				params.parachain_id.unwrap_or(2036).into(),
			)?)?;
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				let id = ParaId::from(cli.run.parachain_id.or(para_id).unwrap_or(2036));
